...
```{r, results='asis', echo=FALSE}
DT::datatable(round(res$tuning.parameters,5), options = list(pageLength = 100))
```
...
```{r}
coef.cv.shim(cv.res, s = "lambda.1se")
coef.cv.shim(cv.res, s = "lambda.min")
coef.cv.shim(cv.res, s = "s56")
```
