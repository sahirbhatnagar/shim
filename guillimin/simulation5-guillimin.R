...
# Created by Sahir,  Oct 14, 2016
# Updated:  
# Notes: This simulation does not include interactions... It is an updated of the 1st simulation I did for the protocol
...
result <- generate_data(p = p, X = X, binary_outcome = TRUE,
...
filename <- tempfile(pattern = paste0(sprintf("%s_%s_rho%.2f_p%1.0f_SNR%.2f_n%1.0f_s0%1.0f_beta%.2f_alpha%.2f",
                     ...
                     tmpdir = paste(Sys.getenv("PBS_O_WORKDIR"),"results-sim5", sep="/"))
                     # tmpdir = "/home/bhatnaga/coexpression/august2016simulation/linear/results-sim5")
...
write.table(final_results %>% t %>% as.data.frame() %>% colnames(),
            file = paste(Sys.getenv("PBS_O_WORKDIR"),"colnames_stab_hydra-sim5-oct20.txt",sep="/"),
            # file  = "/home/bhatnaga/coexpression/august2016simulation/linear/colnames_stab_mammouth-sim5-oct14.txt",
            # file  = filename,
            quote = F,
