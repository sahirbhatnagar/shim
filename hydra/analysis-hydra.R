...
col.names <- fread("~/git_repositories/eclust-simulation-aug2016/hydra/results/colnames_stab_hydra-sept8.txt", header = F)$V1
...
DT <- fread("~/git_repositories/eclust-simulation-aug2016/hydra/results/sim-hydra-results-p5000-sept8", stringsAsFactors = FALSE) %>%
  setnames(col.names)
...
p[SNR==0.2][alphaMean==0.5] %>%
  ...
  theme(legend.position = "top")
# ggsave(paste(Sys.getenv("HOME"),"eclust/simulation/simulation1/plots/TPR_vs_Shat.png", sep = "/"))
ggsave("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/tpr_fpr_snr02_alpha0.5_corr_vs_tom.png",
       width = 13, height = 12)
