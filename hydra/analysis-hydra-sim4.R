...
DT.long2[measure=="AUC"][,boxplot(value)]
...
save_plot(sprintf("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/sim4-oct14/%s_sim4.png","tpr_fpr",05),
...
for (cdist in c("Correlation","TOM")) {
  ...
  save_plot(sprintf("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/sim4-oct14/tpr_fpr_%s_sim4.png",cdist),
...
# for (i in c(0.5,2)) {
#  
#     ggplot(DT.long2[measure=="AUC"][alphaMean==i], 
...
#       legend.stuff.2
#     
#     ggsave(sprintf("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/sim2-sept8/AUC_alpha%g_sim2.png",i),
#            width = 13, height = 12)
#   
# }
...
for (cdist in c("Correlation","TOM")) {
  ...
  p1 <- ggplot(DT.long2[cluster_distance == cdist][measure=="AUC"],
               aes(x = name, y = value, fill = method)) +
    geom_boxplot(position = pd) +
    guides(color = guide_legend(title = "method")) +
    xlab("") +
  ...
  save_plot(sprintf("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/sim4-oct14/AUC_%s_sim4.png",cdist),
            p1, base_aspect_ratio = 1.3, nrow = 1, base_width = 11, base_height = 11)
...
DT.long2[cluster_distance == "TOM"][measure %in% c("jacc","spearman", "pearson")][is.na(value)]
DT.long2[,table(alphaMean)]
...
for (cdist in c("Correlation","TOM")) {
  for (m in c("jacc","pearson","spearman")) {
    ...
    save_plot(sprintf("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/sim4-oct14/%s_%s_sim4.png",m,cdist),
              p1, base_aspect_ratio = 1.3, nrow = 1, base_width = 11, base_height = 11)
...
for (cdist in c("Correlation","TOM")) {
  p1 <- ggplot(DT.long2[cluster_distance == cdist][measure=="CorrectSparsity"][name %ni% c("enet", "lasso")],
               aes(x = name, y = value, fill = method)) +
    geom_boxplot(position = pd) +
    guides(color = guide_legend(title = "method")) +
    xlab("") +
  ...
  save_plot(sprintf("~/git_repositories/eclust-simulation-aug2016/hydra/results/figures/sim4-oct14/CorrectSparsity_%s_sim4.png",cdist),
            p1, base_aspect_ratio = 1.3, nrow = 1, base_width = 11, base_height = 11)
