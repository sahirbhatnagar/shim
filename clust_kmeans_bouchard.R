##################################
# R source code file for analyzing bouchard data
# Created by Sahir, July 3, 2016
# Updated:
# NOTE: Hosted on 'bouchard' branch of eclust repo on github.
# This file is similar to the clust_kmeans.R file on the nihpd branch of 
# the eclust repo
...
meth_probes <- dimnames(placentaALL)[[1]]

## ---- test-stat ----
registerDoMC(cores = 30)
res <- mclapply(meth_probes, function(i) {
...
# kclustData <- mlKmeans(t(placentaALL), 50)
# save(kclustData,file = "kclustData.RData")
t(placentaALL) %>% dim
load("kclustData.RData")
table(kclustData$result) %>% as.matrix
...
clusters[c(1)] %>% dim
pl <- lapply(clusters[c(1)], function(i) cluster_kmeans(data = i, 
                                                    exposure = E,
                                                    response = Y,
                                                    min_cluster_size = 50,
