...
simToyExample <- function(p, n, n0, rho0, rho1) {
  # 4 groups each of size 10: a,b,c,d
  # a,b are unexposed, c,d are exposed
  
  n1 <- n - n0
  
  # First 200 genes:  
  sig0 <- toeplitz(rep(1,200))
  sig0[lower.tri(sig0)] <- rho0
  sig0[upper.tri(sig0)] <- rho0
  
  sig1 <- toeplitz(rep(1,200))
  sig1[lower.tri(sig1)] <- rho1
  sig1[upper.tri(sig1)] <- rho1
  
  b1_ab <- MASS::mvrnorm(n = n0, mu = rep(-2, 200), Sigma = sig0)
  b1_cd <- MASS::mvrnorm(n = n1, mu = rep(2, 200), Sigma = sig1)
  
  # next 50 genes
  sig0 <- toeplitz(rep(1,50))
  sig0[lower.tri(sig0)] <- rho0
  sig0[upper.tri(sig0)] <- rho0
  
  sig1 <- toeplitz(rep(1,50))
  sig1[lower.tri(sig1)] <- rho1
  sig1[upper.tri(sig1)] <- rho1
  
  b2_a <- MASS::mvrnorm(n = n0/2, mu = rep(-1, 50), Sigma = sig0)
  b2_b <- MASS::mvrnorm(n = n0/2, mu = rep(1, 50), Sigma = sig0)
  b2_c <- MASS::mvrnorm(n = n1/2, mu = rep(-1, 50), Sigma = sig1)
  b2_d <- MASS::mvrnorm(n = n1/2, mu = rep(1, 50), Sigma = sig1)
  
  # remaining 750 genes
  b3_abcd <- MASS::mvrnorm(n = n, mu = rep(0, 750), Sigma = diag(750))
  
  X <- cbind(rbind(b1_ab,b1_cd),
             rbind(b2_a,b2_b, b2_c, b2_d),
             b3_abcd) %>%
...
generate_data_toy <- function(p, Xtrain, Xtest,
  ...
  y_train <- alphaSim*mu_pc1 + (1 - alphaSim) * mu_pc2 + rnorm(n)
  DT_train <- as.matrix(cbind(y_train,Xtrain,E))
  colnames(DT_train)[1] <- "Y"
  
  y_test <- alphaSim*mu_pc1 + (1 - alphaSim) * mu_pc2 + rnorm(n)
  DT_test <- as.matrix(cbind(y_test,Xtest,E))
